...
SELECT DESCRICAO_EQUIPAMENTO -- Liste os equipamentos cuja descrição contenha uma palavra ou trecho informado pelo professor.
FROM EQUIPAMENTO
WHERE DESCRICAO_EQUIPAMENTO LIKE '%VERMELHO%';

SELECT ID_CHAMADO, STATUS_CHAMADO -- Exiba os chamados com status "Fechado".
FROM CHAMADO_SUPORTE
WHERE STATUS_CHAMADO = 'FECHADO';

SELECT ID_CHAMADO, PRIORIDADE_CHAMADO -- Liste os chamados com prioridade "Alta".
FROM CHAMADO_SUPORTE
WHERE PRIORIDADE_CHAMADO = 'Alta';

SELECT DESCRICAO_EQUIPAMENTO -- Exiba todos os equipamentos ordenados pela descrição em ordem alfabética.
